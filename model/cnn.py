...
def build_network(X_batch, training_flag):
...
with tf.device(device):
    with tf.variable_scope('network') as current_scope:
        logits = build_network(X_batch, training_flag=True)
        ...
        with tf.control_dependencies(update_ops):
            apply_gradients_op = optimizer.apply_gradients(grad_var_list, global_step)  # this is training op

        current_scope.reuse_variables()
        logits2 = build_network(X_batch, training_flag=False)
        Y_prediction = tf.nn.sigmoid(logits2)  # this is evaluation op
